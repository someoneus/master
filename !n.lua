...
local Button = Tab:CreateButton({
    Name = "Reset Sliders",
    Callback = function()
        -- get character (or wait if it hasn’t spawned yet)
        ...
        -- reset the humanoid properties
        humanoid.WalkSpeed  = 16
        humanoid.JumpHeight = 7.2
        humanoid.HipHeight  = (humanoid.RigType == Enum.HumanoidRigType.R15) and 1.35 or 0
...
-- Create a single section to display the result
local resultSection = Tab:CreateSection("math.result: nil")

-- Create the input box right away
Tab:CreateInput({
    ...
    Flag                     = "MathInput",
    Callback = function(Text)
        -- formats numbers with commas (e.g. 1,234,567)
        ...
        -- sanitize input: only digits, operators, parentheses, dot, and space
...
local Tab = Window:CreateTab("scripts", "archive") -- Title, Image
local Button = Tab:CreateButton({
    Name = "grow a garden",
    Callback = function()
